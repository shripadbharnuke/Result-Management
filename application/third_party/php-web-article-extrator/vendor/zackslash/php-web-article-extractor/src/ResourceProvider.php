	...
	*/
	class ResourceProvider
	{
		...
		*/
		...
		{
			if(is_file($resourceLocation = dirname(__FILE__).DIRECTORY_SEPARATOR."..".DIRECTORY_SEPARATOR."res".DIRECTORY_SEPARATOR.$resourceName)) {
				$this->resourceContent = $this->readResourceToArray($resourceLocation);
			} else {
				$this->resourceContent = array();
			}
		}
		...
		*/
		public function loadResourceDirectory($directoryName)
		{
			...
			foreach (scandir($resourceLocation) as $file) 
			{
				...
				if((($temp = strlen($file) - strlen(".lst") > 0) && strpos($file, ".lst", $temp) !== FALSE) && is_file($resourceLocation.DIRECTORY_SEPARATOR.$file))
				{
					$fileEntry = array();
