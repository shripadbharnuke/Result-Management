...
class Ion_auth_model extends MY_Model
{
	...
	public function get_users_groups($id=FALSE)
	{
		...
		$this->db->where($this->tables['users_groups'].'.'.$this->join['users'], $id);
		$this->db->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id');
		return $this->db->get($this->tables['users_groups']);
	}
	...
	public function get_user($id=FALSE)
	{
		...
		$this->db->select($this->tables['users'].'.id as user_id, email, username, first_name, last_name, company, phone, '.$this->tables['users_groups'].'.'.$this->join['groups'].' as group_id, '.$this->tables['groups'].'.name as group_name, '.$this->tables['groups'].'.code as group_code');
		$this->db->where($this->tables['users'].'.id', $id);
		$this->db->join($this->tables['users_groups'], $this->tables['users_groups'].'.'.$this->join['users'].'='.$this->tables['users'].'.id');
		$this->db->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.id');
