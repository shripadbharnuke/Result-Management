...
abstract class REST_Controller extends CI_Controller {
	...
	protected function _get_app_version() {
		
		$this->load->model('api/devices_m');
		
		$app_versions = $this->devices_m->get_app_version_details()->result_array();
		
		foreach($app_versions as $app_version) {
			if($app_version['device_type'] == 'ios') {
				$this->output->set_header("X-IOS-API-VERSION: ".$app_version['app_version']);
			}
			
			if($app_version['device_type'] == 'android') {
				$this->output->set_header("X-ANDROID-API-VERSION: ".$app_version['app_version']);
			}
		}
	}
    ...
    protected function _detect_input_format()
    {
        ...
        $content_type = $this->input->server('CONTENT_TYPE', true);
    ...
    protected function _detect_output_format()
    {
        ...
        $http_accept = $this->input->server('HTTP_ACCEPT', true);
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name, true)))
        {
    ...
    {
        $lang = $this->input->server('HTTP_ACCEPT_LANGUAGE', true);
        if ($lang === NULL)
        {
            return NULL;
        }
    ...
    protected function _parse_put()
    {
        ...
        else if ($this->input->method() === 'put')
        {
           ...
           $this->_put_args = $this->input->input_stream(NULL, true);
        }
    }
    ...
    protected function _parse_head()
    {
        ...
        parse_str(parse_url($this->input->server('REQUEST_URI', true), PHP_URL_QUERY), $head);
    ...
    protected function _parse_options()
    {
        ...
        parse_str(parse_url($this->input->server('REQUEST_URI', true), PHP_URL_QUERY), $options);
    ...
    protected function _parse_patch()
    {
        ...
        {
            // If no filetype is provided, then there are probably just arguments
            $this->_patch_args = $this->input->input_stream(NULL, true);
        }
    }
    ...
    protected function _parse_delete()
    {
        ...
        {
            $this->_delete_args = $this->input->input_stream(NULL, true);
        }
    }
    ...
    {
        $this->_query_args = $this->input->get(NULL, true);
    }
     ...
     */
    public function query($key = NULL, $xss_clean = true)
    {
        if ($key === NULL)
        {
    ...
    protected function _prepare_basic_auth()
    {
        ...
        $username = $this->input->server('PHP_AUTH_USER', true);
        $http_auth = $this->input->server('HTTP_AUTHENTICATION', true);

        $password = NULL;
        if ($username !== NULL)
        {
            $password = $this->input->server('PHP_AUTH_PW', true);
        }
        elseif ($http_auth !== NULL)
        {
            ...
            if (strpos(strtolower($http_auth), 'basic') === 0)
            {
                ...
                list($username, $password) = explode(':', base64_decode(substr($this->input->server('HTTP_AUTHORIZATION', true), 6)));
            }
        }
    ...
    protected function _prepare_digest_auth()
    {
        ...
        $digest_string = $this->input->server('PHP_AUTH_DIGEST', true);
        if ($digest_string === NULL)
        {
            $digest_string = $this->input->server('HTTP_AUTHORIZATION', true);
        }
    ...
    protected function _check_cors()
    {
        ...
        else
        {
            ...
            $origin = $this->input->server('HTTP_ORIGIN', true);
            if ($origin === NULL)
            {
                $origin = '';
            }
